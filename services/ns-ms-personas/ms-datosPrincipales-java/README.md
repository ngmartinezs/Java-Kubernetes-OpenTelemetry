# Micro servicio ms-datosPrincipales-java
...
```
    ...
    java -jar  ./target/ms-datosPrincipales-java-0.0.1-SNAPSHOT.jar --PORT=8080 --CORS=* --APPVER=1.0 --VERDATA=2023 --ENV=dev --DATABASE=DATABASE --DATABASEURL=DATABASEURL --DATABASEUSERNAME=DATABASEUSERNAME --DATABASEPASSWORD=DATABASEPASSWORD --DATABASEDRIVER=DATABASEDRIVER --DATABASEPOOLSIZE=DATABASEPOOLSIZE --DATABASEPOOLSIZE=DATABASEPOOLSIZE --DATABASETIMEOUT=DATABASETIMEOUT --NAME_COMPONENT=wsLogin
```
...
```
    ...
    java -jar ./target/ms-datosPrincipales-java-0.0.1-SNAPSHOT.jar --PORT=8081 --CORS=* --APPVER=1.0 --VERDATA=2023 --ENV=dev --DATABASE=DATABASE --DATABASEURL=DATABASEURL --DATABASEUSERNAME=DATABASEUSERNAME --DATABASEPASSWORD=DATABASEPASSWORD --DATABASEDRIVER=DATABASEDRIVER --DATABASEPOOLSIZE=DATABASEPOOLSIZE --DATABASEPOOLSIZE=DATABASEPOOLSIZE --DATABASETIMEOUT=DATABASETIMEOUT --OTEL_TRACES_EXPORTER=none --OTEL_METRICS_EXPORTER=oltp --OTEL_LOGS_EXPORTER=otlp --OTEL_EXPORTER_OTLP_ENDPOINT=http://127.0.0.1:4317 --OTEL_EXPORTER_OTLP_LOGS_PROTOCOL=http/json --NAME_COMPONENT=ms-datosPrincipales-java

    Segunda instruccion utiliza un collector remoto

    java -jar ./target/ms-datosPrincipales-java-0.0.1-SNAPSHOT.jar --PORT=8080 --CORS=* --APPVER=1.0 --VERDATA=2023 --ENV=dev --DATABASE=DATABASE --DATABASEURL=DATABASEURL --DATABASEUSERNAME=DATABASEUSERNAME --DATABASEPASSWORD=DATABASEPASSWORD --DATABASEDRIVER=DATABASEDRIVER --DATABASEPOOLSIZE=DATABASEPOOLSIZE --DATABASEPOOLSIZE=DATABASEPOOLSIZE --DATABASETIMEOUT=DATABASETIMEOUT --OTEL_TRACES_EXPORTER=none --OTEL_METRICS_EXPORTER=oltp --OTEL_LOGS_EXPORTER=otlp --OTEL_EXPORTER_OTLP_ENDPOINT=https://apiaksngms.net:443 --NAME_COMPONENT=ms-datosPrincipales-java 
...
```
    java -javaagent:C:/ngmartinezs/Desarrollo/Java/wsLogin/opt/elastic/elastic-apm-agent-1.39.0-javadoc.jar -Delastic.apm.service_name=my-ws -Delastic.apm.server_urls=https://2022e2cc10a94eaa901f591830d5cf96.apm.eastus2.azure.elastic-cloud.com:443 -Delastic.apm.secret_token=IgCOsjV0rQQJ3FCcCu -Delastic.apm.environment=production -Delastic.apm.application_packages=org.example -jar ./target/ms-datosPrincipales-java-0.0.1-SNAPSHOT.jar --PORT=8080 --CORS=* --APPVER=1.0 --VERDATA=2023 --ENV=dev --DATABASE=DATABASE --DATABASEURL=DATABASEURL --DATABASEUSERNAME=DATABASEUSERNAME --DATABASEPASSWORD=DATABASEPASSWORD --DATABASEDRIVER=DATABASEDRIVER --DATABASEPOOLSIZE=DATABASEPOOLSIZE --DATABASEPOOLSIZE=DATABASEPOOLSIZE --DATABASETIMEOUT=DATABASETIMEOUT
...
```
   ...
   docker image rm adockercrapisngms.azurecr.io/ms-datosPrincipales-java:1.0.7 -f 
   ...
   docker image rm acrapisngms.azurecr.io/ms-datosPrincipales-java:1.0.7 -f

   docker build -t acrapisngms.azurecr.io/ms-datosPrincipales-java:1.0.7 --no-cache .
```
...
```
    az acr repository delete  --n acrapisngms.azurecr.io --image ms-datosPrincipales-java:1.0.7 --yes 
    docker push acrapisngms.azurecr.io/ms-datosPrincipales-java:1.0.7
...
```
  kubectl delete deployment ms-datosPrincipales-java-deployment -n ns-ms-seguridad && \
  kubectl delete service ms-datosPrincipales-java-servicio -n ns-ms-seguridad && \
  kubectl apply -f ./k8s/ms-datosPrincipales-java-k8s.yaml && \
  kubectl get pod -A
